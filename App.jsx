...
function AdminStoreProvider({ children }) {
  ...
  function addPromoCode(promo) {
    ...
    supabaseFetch(`promo_codes?select=id`, {
      method: "POST",
      ...
      if (realId) setPromoCodes(prev => prev.map(p => p.id === tempId ? { ...p, id: realId } : p));
    }).catch(err => console.error("Création Supabase (code promo) échouée :", err));
  }
  ...
  // Partage un code promo — dans cette démo sans backend, il n'y a pas de
  // messagerie client à alimenter (elle vit dans l'autre application) ;
  // on se contente donc de confirmer l'action ici. En production (backend
  // partagé), cet appel écrirait le message dans la même base de données
  // que consulte l'app clients/annonceurs.
  function addAdvertiserMessage() {}
            ...
            {showAddPromo && (
              <div className="bg-white rounded-2xl border border-gray-100 p-3.5 space-y-2.5">
                <div className="flex gap-2">
                  ...
                  <button onClick={() => setNewPromoCode(generatePromoCode())}
